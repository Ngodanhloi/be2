...
<main>
    ...
    <section id="accounts-section" class="admin-section">
        ...
        <div class="search-container">
            ...
            <button type="button" id="searchButton" onclick="searchAccounts()">Tìm kiếm</button>
        </div>
        <!-- Button to trigger the popup -->
        <button id="openPopupButton" class="btn btn-primary">Thêm tài khoản</button>
        <!-- Popup form -->
        <div id="popupForm" style="display: none;">
            <form id="addProductForm" action="{{ route('admin.listUser.store') }}" method="POST"
                        ...
                        @foreach ($users as $user)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('admin.listUser.delete', ['user_id' => $user->user_id]) }}" method="POST"
                                ...
                                <div id="editFormContainer-{{ $user->user_id }}" class="edit-form-container" style="display: none;">
                                    <form action="{{ route('admin.listUser.update', $user->user_id) }}" method="post">
                                        ...
                                        <div class="form-group">
                                            <label for="ten">Tên Người Dùng:</label>
                                            <input type="text" id="ten" name="ten" class="form-control" value="{{ $user->name }}"
                                                required>
                                        </div>
                                        <div class="form-group">
                                            <label for="email">Email:</label>
...
<script>
    ...
    function searchAccounts() {
        var searchKeyword = removeVietnameseTones(document.getElementById('searchInput').value.toLowerCase());
        var accounts = document.querySelectorAll('#accounts-section tbody tr');

        var found = false;
        accounts.forEach(function(account) {
            var name = removeVietnameseTones(account.querySelector('td:nth-child(2)').textContent.toLowerCase());
    ...
    // Gắn sự kiện click cho nút tìm kiếm (phòng khi không dùng inline onclick)
