                    ...
                    <div class="col l-4 m-12 s-12">
                        <div class="main__pay">
                            ...
                            @if(session('discount_code'))
                                <p class="main__pay-title total-after-discount">Tổng tiền sau giảm:
                                    {{ number_format(session('total_after_discount')) }}đ</p>
                            @endif
                            <form method="GET" action="{{ route('pay') }}">
                                <input type="hidden" name="totalPrice"
                                    value="{{ session('total_after_discount', $totalPrice) }}">
        ...
        <script>
            ...
            $(document).ready(function () {
                $('#updateCartBtn').on('click', function (e) {
                    e.preventDefault(); // ngăn reload ngay lập tức
                    $('.total-after-discount').hide(); // ẩn phần giảm giá
                    // Gửi ajax cập nhật giỏ hàng hoặc reload nếu cần
                    location.reload(); // nếu muốn reload lại
                });
            });
